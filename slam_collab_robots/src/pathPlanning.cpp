...
/*--------------------------------------------------*/
void drawTrajectory( vector<Point2f> traj, Mat map, Point2f target_pt ){

  for (auto vec : traj)
  {
    circle(map, vec, 1, Scalar(0, 255, 0));
  }
  circle(map, target_pt,2,Scalar(255,0,0),FILLED);
  imwrite("path/path"+ to_string(readRobotId()) +".jpg", map);
  namedWindow("Path "+ to_string(readRobotId()), WINDOW_NORMAL);
  imshow("Path "+ to_string(readRobotId()),map);
...
void drawObstacles(Mat global_map_draw, vector<Point2f> obstacles){
  for (auto vec : obstacles)
  {
    circle(global_map_draw, vec, 3, Scalar(255, 0, 0));
  }
 // circle(global_map_draw, Point(50,49),2, Scalar(0,255,0));
  imwrite("obst_"+ to_string(readRobotId()) +".jpg", global_map_draw);
